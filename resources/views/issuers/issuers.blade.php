...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="px-4 py-4 bg-white shadow-sm sm:px-6 sm:py-6">
        <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <!-- Title and subtitle -->
            <div>
                <h1 class="text-2xl font-light text-gray-800 sm:text-3xl">Issuers Dashboard</h1>
                <p class="text-sm text-gray-500 sm:text-base">Manage item issues and track your contributions</p>
            </div>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-gray-500 ms-1 md:ms-2">Issuers</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="grid grid-cols-1 gap-6 p-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <!-- Add Issue Card -->
        @if (has_permission(96))
        <a href="{{ url('issuers/getgoods') }}" class="group">
            <div class="flex flex-col items-center h-full p-6 transition-all duration-300 transform bg-[#1C1C1E] shadow-sm rounded-xl hover:shadow-lg hover:-translate-y-1">
                ...
                <h3 class="text-lg font-semibold text-center text-white transition-colors group-hover:text-red-300">Get Goods</h3>
                <p class="mt-1 text-xs text-center text-gray-400">Record a new issue for items</p>
            </div>
        </a>
        @endif

        <!-- View My Issues Card -->
        @if (has_permission(97))
        <a href="{{ url('issuers/viewGood') }}" class="group">
            <div class="flex flex-col items-center h-full p-6 transition-all duration-300 transform bg-[#1C1C1E] shadow-sm rounded-xl hover:shadow-lg hover:-translate-y-1">
                ...
                <h3 class="text-lg font-semibold text-center text-white transition-colors group-hover:text-orange-300">View My Issues</h3>
                <p class="mt-1 text-xs text-center text-gray-400">Check all issues you have recorded</p>
            </div>
        </a>
        @endif
