<body class="bg-gray-50">
    ...
    <div class="px-4 py-4 sm:px-6 lg:px-8">
        <div class="flex flex-col space-y-6">
                ...
                <ol class="inline-flex flex-wrap items-center space-x-1 text-sm text-gray-600">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ url('/reports/donations') }}" class="transition-colors hover:text-blue-600">Donations History</a>
                        </div>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="font-medium text-gray-700">Donation Details</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white shadow sm:rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 mt-6 gap-y-6 sm:grid-cols-2 sm:gap-x-6">
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Donator Name</h3>
                            <p class="mt-1 text-lg text-gray-900">{{ $donation->donator->name ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Donation Date</h3>
                            <p class="mt-1 text-lg text-gray-900">{{ \Carbon\Carbon::parse($donation->donation_date)->format('d M Y') }}</p>
                        </div>
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Total Quantity</h3>
                            <p class="mt-1 text-lg text-gray-900">{{ $donation->total_quantity }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Donated Items Table -->
            <div class="mt-6">
                <h2 class="text-xl font-semibold text-gray-900">Donated Items</h2>
                <div class="mt-4 overflow-x-auto">
                    <table class="min-w-full bg-white border border-gray-200 rounded-md">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($donation->items as $donationItem)
                                <tr>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{{ $donationItem->item->item_name ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{{ $donationItem->item->item_code ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{{ $donationItem->quantity }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                        <img src="{{ $donationItem->item->image_url }}" alt="{{ $donationItem->item->item_name ?? 'Item Image' }}" class="object-cover w-16 h-16 rounded-md">
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-sm text-center text-gray-500">No items found for this donation.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <!-- Back Button -->
            <div class="mt-6">
                <a href="{{ url('/reports/donations') }}" class="inline-flex items-center px-4 py-2 text-white bg-gray-600 rounded-md hover:bg-gray-700">
